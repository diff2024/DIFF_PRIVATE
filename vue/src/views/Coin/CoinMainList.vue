				...
				<v-card-text>
					<v-row style="padding-left:15px; padding-top:0px; padding-bottom:0px;">
						...
						<v-col lg="10" md="10" sm="8" cols="8" style="padding-top:0px; padding-bottom:0px; text-align:right;">
							...
							<v-btn depressed dark small color="red" style="float:right; margin-top:17px; margin-left:10px;" :to="{ name: 'youtubeupbit_240'}" target="_blank">
								<v-icon small>tv</v-icon>&nbsp;<span id="btn_live_240" style="padding-bottom:2px;">240 Live</span>
							</v-btn>
							<v-btn depressed dark small color="red" style="float:right; margin-top:17px; margin-left:10px;" :to="{ name: 'youtubeupbit_60'}" target="_blank">
								<v-icon small>tv</v-icon>&nbsp;<span id="btn_live_60" style="padding-bottom:2px;">60 Live</span>
							</v-btn>
							<v-btn depressed dark small color="red" style="float:right; margin-top:17px; margin-left:10px;" :to="{ name: 'youtubeupbit_15'}" target="_blank">
								<v-icon small>tv</v-icon>&nbsp;<span id="btn_live_15" style="padding-bottom:2px;">15 Live</span>
							</v-btn>
							<v-btn depressed dark small color="red" style="float:right; margin-top:17px;" :to="{ name: 'youtubeupbit_5'}" target="_blank">
					...
					<v-row justify="center">
						<v-dialog v-model="excelModal" persistent max-width="480" @keydown.esc="excelModal = false">
							<v-card>
							...
							<v-card-text style="padding-bottom:0px;">
								...
								<v-row v-for="(coin, index) in coinInfo" v-bind:key="index">
									<v-col xl="1" md="1" sm="1" cols="1" align-self="center" style="padding-top:0px; padding-bottom:0px;">
										<v-checkbox style="left:50%;float:left;" v-model="coin.coin_YN" v-bind:disabled="coin.coin_ticker == 'KRW-BTC'" true-value="Y" false-value="N" />
									</v-col>
		...
		makeExcel() {
			...
			if(this.excel_std_date == this.excel_end_date){
				...
				.then(response => {
					...
					link.href = url;
					link.setAttribute('download', '업비트_Daily_'+this.excel_std_date+'.xlsx');
				...
				axios.get('/Upbit/ExcelMake_Monthly',{
				params: {
						std_date: this.excel_std_date,
						end_date: this.excel_end_date,
				...
				.then(response => {
					...
					const link = document.createElement('a');
					link.href = url;
					link.setAttribute('download', '업비트_Monthly_'+this.excel_std_date+'_'+this.excel_end_date+'.xlsx');
					document.body.appendChild(link);
					link.click();
				});
			}else{
				...
				.then(response => {
					...
					const link = document.createElement('a');
					link.href = url;
					link.setAttribute('download', '업비트_Weekly_'+this.excel_std_date+'_'+this.excel_end_date+'.xlsx');
					document.body.appendChild(link);
					link.click();
				});
			}
