				...
				<v-card-text>
					<template>
						<ag-grid-vue 
							...
							:rowData="rowData"
							:animateRows="true"
							overlayNoRowsTemplate="바이비트에서 데이터 가져오는중...">
						</ag-grid-vue>
					</template>
				</v-card-text>
			</v-card>
	...
	methods: {
		makeReport(){
			if(this.$route.params.date !== undefined){
				...
				Swal.fire({
					icon: 'question',
					text: " 바이비트 " + this.today_date + " 일일 리포트를 생성 하시겠습니까?",
		...
		makeExcel() {
			...
			if(this.excel_std_date == this.excel_end_date){
				axios.get('/Bybit/ExcelMake_Daily',{
				params: {
				...
				.then(response => {
					...
					link.href = url;
					link.setAttribute('download', '바이비트_Daily_'+this.excel_std_date+'.xlsx');
				...
				axios.get('/Bybit/ExcelMake_Monthly',{
				params: {
						std_date: this.excel_std_date,
						end_date: this.excel_end_date,
				...
				.then(response => {
					...
					const link = document.createElement('a');
					link.href = url;
					link.setAttribute('download', '바이비트_Monthly_'+this.excel_std_date+'_'+this.excel_end_date+'.xlsx');
					document.body.appendChild(link);
					link.click();
				});
			}else{
				axios.get('/Bybit/ExcelMake_Weekly',{
				params: {
						std_date: this.excel_std_date,
						end_date: this.excel_end_date,
						coin_ticker: coin_data
				...
				.then(response => {
					...
					const link = document.createElement('a');
					link.href = url;
					link.setAttribute('download', '바이비트_Weekly_'+this.excel_std_date+'_'+this.excel_end_date+'.xlsx');
					document.body.appendChild(link);
					link.click();
				});
			}
