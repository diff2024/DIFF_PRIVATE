...
export default {
	...
	created () {
		...
		axios.get('/BinanceFutures/CoinInfo')
		.then(response => {
	...
	mounted (){
		...
		if(this.$route.params.date !== undefined){
			...
			if(Number(tmp1) < 10 && tmp1.length == 1){
				tmp1 = '0'+tmp1
			}
	...
	methods: {
		makeReport(){
			if(this.$route.params.date !== undefined){
				...
				Swal.fire({
					icon: 'question',
					text: " 바이낸스 선물 " + this.today_date + " 일일 리포트를 생성 하시겠습니까?",
		...
		makeExcel() {
			...
			if(this.excel_std_date == this.excel_end_date){
				axios.get('/BinanceFutures/ExcelMake_Daily',{
				params: {
				...
				.then(response => {
					...
					link.href = url;
					link.setAttribute('download', '바이낸스_선물_Daily_'+this.excel_std_date+'.xlsx');
				...
				axios.get('/BinanceFutures/ExcelMake_Monthly',{
				params: {
						std_date: this.excel_std_date,
						end_date: this.excel_end_date,
				...
				.then(response => {
					...
					const link = document.createElement('a');
					link.href = url;
					link.setAttribute('download', '바이낸스_선물_Monthly_'+this.excel_std_date+'_'+this.excel_end_date+'.xlsx');
					document.body.appendChild(link);
					link.click();
				});
			}else{
				axios.get('/BinanceFutures/ExcelMake_Weekly',{
				params: {
						std_date: this.excel_std_date,
						end_date: this.excel_end_date,
						coin_ticker: coin_data
				...
				.then(response => {
					...
					const link = document.createElement('a');
					link.href = url;
					link.setAttribute('download', '바이낸스_선물_Weekly_'+this.excel_std_date+'_'+this.excel_end_date+'.xlsx');
					document.body.appendChild(link);
					link.click();
				});
			}
		...
		makeData () {
			...
			this.isLoading = true;
			axios.get('/BinanceFutures/MainGridList',{
			params: {
