...
<template>
    <v-flex xs12 sm12 md12>
			<v-card>
				<v-card-text>
					<template>
						<h1 id="title" style="font-weight:bold; padding-bottom:20px; color:black;">업비트 코인 1시간</h1>
						<ag-grid-vue 
...
export default {
	...
	methods: {
		makeData () {
			this.rowData = [];
			axios.get('/Main/upbit_60m')
			.then(response => {
				console.log('upbit_60m     ' + String(response.data.length));
