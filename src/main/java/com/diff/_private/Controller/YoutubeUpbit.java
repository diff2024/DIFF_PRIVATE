...
public class YoutubeUpbit {
		...
		for(int x=0; x<CoinList.size(); x++) {
	        ...
	        if((jObject.getString("s")).equals("ok")) {
	        	...
	        	for (int i = 0; i < jArray_t.length(); i++) {
	        		...
	        		if(!c_price.contains(".") && (new BigDecimal(c_price).abs()).compareTo(new BigDecimal("999")) == 1) {
	        		...
	        		if(!volume_price.contains(".") && (new BigDecimal(volume_price).abs()).compareTo(new BigDecimal("999")) == 1) {
	        			format_volume_price = volume_price.replaceAll("\\B(?=(\\d{3})+(?!\\d))", ",");
	        		}
	        		String o_c_rate = (BD_O_C_RATE.setScale(2, RoundingMode.FLOOR)).toString() + '%';
	        		String o_c_subtract = (BD_O_C_SUBTRACT.toString());
	        		if(!(BD_O_C_SUBTRACT.toString()).contains(".") && (new BigDecimal(BD_O_C_SUBTRACT.toString()).abs()).compareTo(new BigDecimal("999")) == 1) {
	...
	@GetMapping(path = "/OC_MIN5")
	public List<HashMap<String, String>> OC_MIN5() throws Exception{
		String START_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + START_DATETIME + "] OC_MIN5 시작");
		String GIJUN_DATETIME = "";
		LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        String DateNow = now.format(formatter);
		...
		Collections.sort(APICoinList, new Comparator<HashMap<String,String>>(){
			public int compare(HashMap<String,String> map1, HashMap<String,String> map2){
				if((new BigDecimal((map2.get("o_c_rate")).replace("%",""))).compareTo(new BigDecimal((map1.get("o_c_rate")).replace("%",""))) == 1) return 1;
                else if((new BigDecimal((map2.get("o_c_rate")).replace("%",""))).compareTo(new BigDecimal((map1.get("o_c_rate")).replace("%",""))) == 0) return 0;
                else return -1;
			}
		});
		...
		String END_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + END_DATETIME + "] OC_MIN5 종료");
		
		return APICoinList;
	}
	
	@GetMapping(path = "/OC_MIN15")
	public List<HashMap<String, String>> OC_MIN15() throws Exception{
		String START_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + START_DATETIME + "] OC_MIN15 시작");
		...
		String END_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + END_DATETIME + "] OC_MIN15 종료");
		
		return APICoinList;
	}
	
	@GetMapping(path = "/OC_MIN60")
	public List<HashMap<String, String>> OC_MIN60() throws Exception{
		String START_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + START_DATETIME + "] OC_MIN60 시작");
		...
		String END_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + END_DATETIME + "] OC_MIN60 종료");
		
		return APICoinList;
	}
	
	@GetMapping(path = "/OC_MIN240")
	public List<HashMap<String, String>> OC_MIN240() throws Exception{
		String START_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + START_DATETIME + "] OC_MIN240 시작");
		...
		String END_DATETIME = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format((Calendar.getInstance()).getTime());
		System.out.println("[업비트][" + END_DATETIME + "] OC_MIN240 종료");
