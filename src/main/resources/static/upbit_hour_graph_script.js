...
function drawChart() {
    ...
    fetch('/Main/UPBIT_BTC_HOUR_Ticker_Comparison?date='+date)
    ...
    .then((list) => {
        ...
        for(var x=0; x<Number(main_ranking); x++){
            ...
            var options = {
                title: date_kst+' BTC / '+(String(data_list[0]['rank'+String(Number(x)+1)+'_coin_ticker'])).replace('KRW-', '')+' 1시간봉 증감율(%)',	
                titleTextStyle: {
                    fontName: '맑은 고딕',
                    fontSize: 20,
                    color: 'black',
            ...
            var chart = new google.visualization.LineChart(document.getElementById('upbit_hour_rank'+String(Number(x)+1)+'_graph'));
